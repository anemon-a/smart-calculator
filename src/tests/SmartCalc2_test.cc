...
TEST(deposit, test1) {
  using namespace s21;
  Deposit calc;
  Deposit::ListDA ReplenishmentsList;
  ReplenishmentsList.insert(std::make_pair(Date(30, 03, 2023), 20000.00));
  ReplenishmentsList.insert(std::make_pair(Date(04, 05, 2023), 60000.00));
  Deposit::ListDA PartialWithdrawalsList;
  ...
  ASSERT_NEAR(calc.GetResult().deposit_amount_by_end, 2472058.16, kAcc);
  ASSERT_NEAR(calc.GetResult().accrued_interest, 1062058.16, kAcc);
...
TEST(deposit, test2) {
  ...
  Deposit calc;
  Deposit::ListDA ReplenishmentsList;
  ReplenishmentsList.insert(std::make_pair(Date(28, 01, 2024), 7777.77));
  Deposit::ListDA PartialWithdrawalsList;
  calc.CalculateDeposit(1000000, Date(13, 3, 2023), 20, 2, 8.13, 0.0, 1, 0,
                        ReplenishmentsList, PartialWithdrawalsList);
  ASSERT_NEAR(calc.GetResult().deposit_amount_by_end, 1007777.77, kAcc);
  ASSERT_NEAR(calc.GetResult().accrued_interest, 1638095.60, kAcc);
  ASSERT_NEAR(calc.GetResult().tax_amount, 0.0, kAcc);
}
...
TEST(deposit, test3) {
  ...
  calc.CalculateDeposit(1000000, Date(13, 3, 2023), 2000, 0, 8.13, 0.0, 0, 0,
                        ReplenishmentsList, PartialWithdrawalsList);
  ASSERT_NEAR(calc.GetResult().deposit_amount_by_end, 1007777.77, kAcc);
  ASSERT_NEAR(calc.GetResult().accrued_interest, 440977.16, kAcc);
  ASSERT_NEAR(calc.GetResult().tax_amount, 0.0, kAcc);
}
